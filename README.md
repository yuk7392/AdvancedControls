...
컨트롤마다 테마 색을 그대로 따르되, 필요하면 디자이너에서 색을 개별로 덮어쓸 수 있습니다. 속성 창의 `AdvancedControlOptions › Palette`에서 강조색(Accent) · 면색(Surface) · 테두리 · 글자색 등을 지정하면 그 색만 테마 대신 쓰이고, 비워 두면 테마를 따릅니다. 버튼 · 배지 · 알림 등은 `Context` 색을 고르면 그 한 색에서 hover · 눌림 · 글자색 · 옅은 배경이 자동으로 파생됩니다(비우면 테마 강조색). 모서리 반경 · 테두리 두께 · 선 모양(실선/점선) · 그라데이션 각도 · 그림자 · 포커스 글로우는 `AdvancedControlOptions › Styling`에서 펼쳐 조정합니다. 체크박스와 라디오 버튼은 `ButtonStyle`을 켜면 눌리는 버튼 모양으로 그려집니다.
...
Every control follows the theme by default, but any color can be overridden per control from the designer. Set an accent, surface, border, or text color under `AdvancedControlOptions › Palette` and only that color replaces the theme value; leave it blank to keep following the theme. For buttons, badges, alerts, and the like, pick a `Context` color and the hover, pressed, text, and subtle-background shades are derived from that single color (leave it blank to follow the theme accent). Corner radius, border width, dash style, gradient angle, shadow, and focus glow are expandable under `AdvancedControlOptions › Styling`. Check boxes and radio buttons can be drawn as pressable buttons by turning on `ButtonStyle`.
